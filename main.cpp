...
void State::initFramebuffer(){
    ...
    for (size_t i = 0; i < swapchainImages.size(); i++) {
        ...
        VkMemoryAllocateInfo depthAlloc = {
            ...
            .memoryTypeIndex = findMemoryType(memoryProperties, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depthReqs.memoryTypeBits),
        };
...
void State::initShaders(){
    ...
    VkMemoryAllocateInfo allocInfo = {
        ...
        .memoryTypeIndex = findMemoryType(memoryProperties, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, memReqs.memoryTypeBits),
    };
...
void State::buildPipeline(VkShaderModule fragModule){
    ...
    std::array<VkVertexInputAttributeDescription, 4> vAttributeDescriptions = {{
...
void State::initVulkan(){
    ...
    initFramebuffer();
    initResources();
    model.initGltf(MODEL_PATH, logicalDevice, queue, memoryProperties, commandPool, width, height);
    model.createResources(swapchainImages, swapchainImageFormat, renderPass, swapchainImageViews, depthImageViews);
    initShaders();
    initUniforms();
}
...
void State::updateUniforms(){
    ...
    model.updateBuffers(frameIndex, mvp);
    //printMVP(mvp);
}
...
void State::renderLoop(){
    ...
    while(running){
        ...
        updateUniforms();
        model.recordRenderPass(imgIdx, frameIndex, commandBuffers, runtime);
...
void State::exit(){
    for(size_t i{0u}; i<swapchainImages.size(); i++){
        if(i < framebuffers.size()) vkDestroyFramebuffer(logicalDevice, framebuffers[i], NULL);
