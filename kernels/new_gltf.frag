...
const float STOCHASTICBLUR_RADIUS = 24.0;
const int STOCHASTIC_SAMPLES = 20;
...
vec3 stochasticBlurColor(vec3 srcColor, vec2 st, float radiusPx) {
    vec3 gradX = dFdx(srcColor);
    vec3 gradY = dFdy(srcColor);

    vec3 accum = srcColor;
    for (int i = 0; i < STOCHASTIC_SAMPLES; ++i) {
        float seed = pc.u_time * 0.173 + float(i) * 13.71;
        vec2 rnd = random2(vec3(st * pc.resolution.xy, seed));
        float a = TAU * rnd.x;
        float r = sqrt(rnd.y) * radiusPx;

        float sx = cos(a) * r;
        float sy = sin(a) * r;
        accum += srcColor + gradX * sx + gradY * sy;
    }
    return max(accum / float(STOCHASTIC_SAMPLES + 1), vec3(0.0));
}
...
void main() {
    ...
    vec3 colorLinear = ambient + Lo + emissive;

    vec2 st = gl_FragCoord.xy / pc.resolution;
    vec3 blurredLinear = stochasticBlurColor(colorLinear, st, STOCHASTICBLUR_RADIUS);
    vec3 color = pow(blurredLinear, vec3(1.0 / 2.2));
